...
void PRINTF(int i,char *pbuf)
{
	char path[30]={};
	sprintf(path ,"./flag.txt");
	FILE* fd = fopen(path,"wb");
	fwrite(pbuf, 1, sizeof(int)*i, fd);
	fclose(fd);
}
...
int MSR_GET_FORM(const char* ip, void* msg, int size)
{

	int sockfd = socket_connect(ip, 8117, 1000);
	if(-1 == sockfd)
		return 0;
...
int main()
{
	...
	if(0 == strncmp(pbuf, "pause", 6))//暂停
	{
		char msg[2]  = "0";
		MSR_GET_FORM("127.0.0.1", (void *)msg, sizeof(msg));
	}
	else if(0 == strncmp(pbuf, "play", 5))//播放
	{
		char msg[2]  = "1";
		MSR_GET_FORM("127.0.0.1", (void *)msg, sizeof(msg));
	}
	else if(0 == strncmp(pbuf, "next", 5))//下一首
	{
		char msg[2]  = "2";
		MSR_GET_FORM("127.0.0.1", (void *)msg, sizeof(msg));
	}
	else if(0 == strncmp(pbuf, "prev", 5))//上一首
	{
		char msg[2]  = "3";
		MSR_GET_FORM("127.0.0.1", (void *)msg, sizeof(msg));
	}
	else if(0 == strncmp(pbuf, "stop", 5))//退出
	{
		char msg[2]  = "4";
		MSR_GET_FORM("127.0.0.1", (void *)msg, sizeof(msg));
	}
